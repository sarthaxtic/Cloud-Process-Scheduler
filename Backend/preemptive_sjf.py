def preemptive_sjf(processes):
    ...
    while len(completed) < len(processes):
        ...
        if ready:
            shortest = min(ready, key=lambda x: remaining[x['processId']])
            ...
            if remaining[pid] == 0:
                ...
                turnaround_total += time + 1 - shortest['arrivalTime']
                waiting_total += time + 1 - shortest['arrivalTime'] - shortest['burstTime']
        else:
